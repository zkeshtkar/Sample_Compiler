...
/* REAL NUMBERS */
RealLiteral  = ("-")?{Float}
Float    =  ({digit}*({digit} \. | \. {digit} | {digit}){digit}*)("f" | "F")?
...
<YYINITIAL> {
    ...
    /* NUMBERS */
    {IntLiteral}        {if (yytext().endsWith("L"))
                            return  symbol("int_const", Long.valueOf(yytext()));
                         else
                            return symbol("int_const", Integer.valueOf(yytext()));}
    {HexLiteral}        {return symbol("int_const", yytext());}
    {RealLiteral}       {if (yytext().endsWith("F"))
                            return symbol("real_const", Float.valueOf(yytext()));
                         else

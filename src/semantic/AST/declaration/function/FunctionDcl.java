package semantic.AST.declaration.function;
...
public class FunctionDcl implements Declaration {
    ...
    public void addParameter(String name, LocalDSCP dscp) {
        ...
        if (dscp instanceof LocalVarDSCP)
            ...
            paramTypes.add(Type.getType(repeatedArray + dscp.getType()));
        }
    }
    ...
    private void setSig() {
        ...
        for (ParamPair param :
            ...
            if (param.dscp instanceof LocalArrDSCP){
                String repeatedArray = "";
                for (int i = 0; i < ((LocalArrDSCP) (param.dscp)).getDimNum(); i++) {
                    repeatedArray += "[";
                }
                type = Type.getType(repeatedArray + param.dscp.getType());
            }
            paramTypes.add(type);
